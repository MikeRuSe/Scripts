...
If ($opcion -eq "4"){
         ...
         ## Establecemos la dirección del archivo que queremos cifrar
        $cifrar= Read-Host 'Introduzca la dirección del archivo a cifrar (sin "/" al final)'
        $cifrar1= Read-Host "Introduzca el nombre del archivo a cifrar"
         ## Introducimos una contraseña para proteger el archivo
        $Contraseña1 = (ConvertTo-SecureString (Read-Host "Escriba la contraseña con la que va a cifrar el archivo") -AsPlainText -Force)
        $Contraseña2 = (ConvertTo-SecureString (Read-Host "Repita la contraseña con la que va a cifrar el archivo") -AsPlainText -Force) 
        If ($Contraseña1 -ceq $Contraseña2){
             ## Convertimos la contraseña a SecureString
            $ContraseñaCifrada = (ConvertTo-SecureString $Contraseña2 -AsPlainText -Force)
             ## Protegemos el archivo con la contraseña 
                Set-Content -Path "$cifrar/$cifrar1" -Value $ContraseñaCifrada 
                $hash1= $contenido
                Set-Content -Path "$cifrar/$cifrar1" -Value $ContraseñaCifrada
            }
        Else{
            Write-Host -ForegroundColor Red "Las contraseñas no coinciden"
            }
        }
        Else{
If ($opcion -eq "5"){
