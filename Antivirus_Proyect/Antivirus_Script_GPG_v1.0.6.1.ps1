...
If ($opcion -eq "3"){
     ...
     Else{
If ($opcion -eq "4"){
        Write-Host -ForegroundColor Yellow "RECUERDE QUE DEBE TENER INSTALADO GNU GPG2 EN: "  -NoNewline; Write-Host -ForegroundColor Green '"C:\Program Files (x86)\GNU\GnuPG\gpg2.exe"'
        ...
        If ($modulo -eq "No"){
             ## Instalación del módulo PGP
            $enlace = 'https://raw.githubusercontent.com/adbertram/Random-PowerShell-Work/master/Security/GnuPg.psm1'
            $carpetademodulo = 'C:\Program Files\WindowsPowerShell\Modules\GnuPg'
            $noexiste = New-Item -Path $carpetademodulo -Type Directory
            Invoke-WebRequest -Uri $enlace -OutFile (Join-Path -Path $carpetademodulo -ChildPath 'GnuPg.psm1')
           }
        If ($modulo -ne "No"){
        ...
        if ($valorDE -eq "1"){
             ## Establecemos la dirección de la carpeta que queremos cifrar
            $cifrar= Read-Host  'Introduzca la dirección de la carpeta que queremos cifrar (sin "/" al final)'
             ## Introducimos una contraseña para proteger el archivo
            $Contraseña1 = (Read-Host "Escriba la contraseña con la que va a cifrar la carpeta")
            Clear-Host
            $Contraseña2 = (Read-Host "Repita la contraseña con la que va a cifrar la carpeta")
            Clear-Host
            Write-Host -ForegroundColor  " "
            Write-Host "                " -NoNewline; Write-Host "==================== CIFRADOR DE ARCHIVOS CON PGP ====================" -ForegroundColor Green
            If ($Contraseña1 -ceq $Contraseña2){
                ## Preguntamos si queremos guardar la contraseña en un archivo
                Write-Host -ForegroundColor Yellow "¿Desea guardar la contraseña en un archivo?"
                $pregunta1= (Read-Host "Si/No")
                if ($pregunta1 -eq "Si"){
                    ...
                    $Contraseña1  > "$carpetakey/$archivokey.txt"
                }
                else{
                    Write-Host -ForegroundColor Red "No se guardará la contraseña, ¿está seguro de no querer almacenar la contraseña?"
                    $pregunta2= (Read-Host "Si/No")
                    if ($pregunta2 -eq "No"){
                        $carpetakey = Read-Host "Seleccione una carpeta para guardar la clave"
                        $archivokey = Read-Host "Introduzca el nombre del archivo a guardar (sin extensión ej .ps1)"
                        $Contraseña1  > "$carpetakey/$archivokey.txt"
                            ...
                            Write-Host -ForegroundColor Red "No se guardará la contraseña"
                        }
                    }
                Add-Encryption -FolderPath $cifrar -Password "$Contraseña1"
                Start-Sleep -Milliseconds 250
                Write-Host -ForegroundColor Green "La carpeta $cifrar se encriptó correctamente"
                }
            }
        if ($valorDE -eq "2"){
            ...
            Start-Sleep -Milliseconds 100
              ## Manual
             if ($claveDes -eq "1"){
                Write-Host -ForegroundColor Yellow "Inserte la clave"
                $claveDesencriptado= Read-Host "Inserte"
                Remove-Encryption -FolderPath ($cifrar/$cifrar1) -Password "$claveDesencriptado"
